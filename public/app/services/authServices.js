...
.factory('Auth', function($http, AuthToken, $location){
 ...
 // Function to get current user's data
    authFactory.getUser = function() {
        ...
        if (AuthToken.getToken()) {
            return $http.post('/api/currentusr') // Return user's data	
        } else {
...
.factory('AuthInterceptors', function($window) {
    ...
    authInterceptorsFactory.request = function(config) {
        var token = $window.localStorage.getItem('token'); // Check if a token is in local storage
